...
public:
    vector<int> pre;
    int preIdx;
    unordered_map<int, int> map; // val -> idx 的映射
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        this->pre = preorder;
        preIdx = 0;
        for(int i=0;i<inorder.size();i++){
        ...
        return buildSubTree(0,preorder.size()-1);
    }

    TreeNode* buildSubTree(int l,int r){
        if(l>r){
            return nullptr;
        }
        ...
        TreeNode* root = new TreeNode(rootVal);
        root->left = buildSubTree(l,inorderIdx-1);
        root->right = buildSubTree(inorderIdx+1,r);
        return root;
    }
};
// 测试代码
